...
public class AdicionarAPE extends JFrame {
	...
	public AdicionarAPE(DataManager data, APE ape){
			...
			public void mouseClicked(MouseEvent arg0) {
				if(!temCampoVazio()){
					try {
						...
						JOptionPane.showMessageDialog(null, "APE Editada!");
						setVisible(false);
					} catch (IOException e) {
						JOptionPane.showMessageDialog(null, "Falha ao Editar APE");
						e.printStackTrace();
					}
				}
				else{
		...
		txtFieldDataEnvio.setText(ape.getDataDeEnvio());
		txtFieldDataVencimento.setText(ape.getDataDeVencimento());
		txtFieldDescricao.setText(ape.getDescricao());
		txtFieldFornecedor.setText(ape.getFornecedor());
		txtFieldNumBatch.setText(ape.getNumBatch());
		txtFieldNumPedido.setText(ape.getNumPedido());
		txtFieldValor.setText(ape.getValor());
	...
	public AdicionarAPE(DataManager data) {
		setTitle("Cadastrar Nova APE");
		this.data=data;
		setDefaultCloseOperation(JFrame.HIDE_ON_CLOSE);
		setBounds(100, 100, 316, 336);
